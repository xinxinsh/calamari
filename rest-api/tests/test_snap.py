...
from calamari_rest.views.v2 import SnapViewSet
from calamari_rest.views.v2 import SNAP, POOL
...
class TestSnap(TestCase):
    ...
    def test_list(self):
        ...
        response = self.cmvs.list(self.request, 12345, 2, 'test')
        self.assertEqual(response.status_code, 200)
 
    def test_retrieve(self):
        self.request.method = 'GET'
        response = self.cmvs.retrieve(self.request, 12345, 2, 'test', 'snap')
        self.assertEqual(response.status_code, 200)

    def test_update(self):
        self.request.method = 'PUT'
        self.request.DATA = {'name': 'snap-rename'}
        response = self.cmvs.update(self.request, 12345, 2, 'test', 'snap')
        self.assertEqual(response.status_code, 202)

    def test_delete(self):
        self.request.method = 'DELETE'
        response = self.cmvs.destroy(self.request, 12345, 2, 'test', 'snap')
        self.assertEqual(response.status_code, 202)

    def test_create(self):
        self.request.method = 'POST'
        self.request.DATA = {'name': 'snap-test'}
        response = self.cmvs.create(self.request, 12345, 2, 'test')
        self.assertEqual(response.status_code, 202)
